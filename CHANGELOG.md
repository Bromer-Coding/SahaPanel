...
- **Supabase — bulunan ve giderilen sorun:** Postgres loglarında her 60 saniyede bir tekrarlayan `current transaction is aborted` hatası tespit edildi. Kök neden: uygulama kodu değil, Supabase'in kendi iç izleme bağlantısı (`postgres_exporter`, `supabase_admin`) takılı kalmıştı. Kullanıcının onayıyla Supabase Dashboard'dan proje restart edildi (Claude in Chrome ile); restart sonrası hata tamamen kesildi, `pg_stat_activity` temiz, Vercel runtime hataları sıfır. Bkz. [TODO.md](TODO.md).
